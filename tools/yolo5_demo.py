...
def make_parser():
    ...
    parser.add_argument("--conf", default=.4, type=float, help="test conf")
    parser.add_argument("--nms", default=.5, type=float, help="test nms threshold")
    parser.add_argument("--tsize", default=(640, 640), type=tuple, help="test img size")
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        #print("here is ret value",ret_val)
        if ret_val:
            outputs, img_info = predictor.inference(frame, timer)
           # print("here is the outputs",outputs)
            if outputs[0] is not None:
                online_targets = tracker.update(outputs[0], [img_info['height'], img_info['width']],size)
                online_tlwhs = []
                online_ids = []
...
def main( args):
    ...
    predictor = Predictor(model, args.num_classes,args.conf,args.nms,args.tsize,args.fp16,args.device,trt_file, decoder)
